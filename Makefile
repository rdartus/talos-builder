...
SYSTEM_EXTENSION_IMAGE_ARGS := $(foreach e,$(EXTENSIONS),--system-extension-image=$(e))
...
installer:
	cd "$(CHECKOUTS_DIRECTORY)/talos" && \
		$(MAKE) \
			...
			INSTALLER_ARCH=arm64 PLATFORM=linux/arm64 \
			IMAGER_ARGS="--overlay-name=rpi5 --overlay-image=$(REGISTRY)/$(REGISTRY_USERNAME)/sbc-raspberrypi5:$(SBCOVERLAY_TAG) $(SYSTEM_EXTENSION_IMAGE_ARGS)" \
